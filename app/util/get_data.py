...
RSS = ['http://rss.cnn.com/rss/cnn_topstories.rss',
       ...
       'http://rss.wn.com/English/international-news',
...
RSS_AGG = ['http://digg.com/rss/top.rss',
	   ...
	   'http://www.google.com/url?q=http%3A%2F%2Fwww.newsvine.com%2F_feeds%2Frss2%2Findex&sa=D&sntz=1&usg=AFQjCNE26pqKvCHuYOcAGGuuBHodKa2zdw']
...
def url_content(url_primary,url_secondary=''):
	...
	try:
		...
		print 'Problem with opening url in url_content'
		return data
...
def requestRssData(url, google=False, newsvine=False, fark=False):
	...
	for i in content.entries:
		...
		if google:
			...
			dicData['title'] = dictData['title'].split('[')[0]
			try:
